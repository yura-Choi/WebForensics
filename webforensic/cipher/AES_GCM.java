...
{
    private static int BLOCK_SIZE_AES = 16;
            ...
            byte[] mKey)
    {
        ...
        aes.AES_KeySched(mKey, rKey);

        aes.AES_Encrypt(H, H, rKey);
        ...
        if (nonceLen == 12)
        {
            ...
            aes.AES_Encrypt(Z, GCTR_in, rKey);
        }
//        else
//        {
//            for (i = 0; i < nonceLen; i += BLOCK_SIZE_AES)
//            {
//                ZERO128(tmp);
//
//                if ((nonceLen - i) < 16)
...
//                else
//                    Byte2Word(tmp, nonce, i, BLOCK_SIZE_AES);
//
...
//        }
        ...
        for (i = 0; i < ptLen; i += BLOCK_SIZE_AES)
        {
            ZERO128(tmp);

            INCREASE(GCTR_in);

            aes.AES_Encrypt(GCTR_out, GCTR_in, rKey);
            ...
            {
                Byte2Word(tmp, pt, i, BLOCK_SIZE_AES);
                XOR128(GCTR_out, GCTR_out, tmp);
                Word2Byte(ct, i, GCTR_out, BLOCK_SIZE_AES);
            }
        }

        for (i = 0; i < aadLen; i += BLOCK_SIZE_AES)
        {
            ZERO128(GHASH_in);
            ...
            if ((aadLen - i) < 16)
                ...
                Byte2Word(GHASH_in, aad, i, BLOCK_SIZE_AES);
        ...
        for (i = 0; i < ptLen; i += BLOCK_SIZE_AES)
        {
            ...
            if ((ptLen - i) < 16)
                ...
                Byte2Word(GHASH_in, ct, i, BLOCK_SIZE_AES);
...
//        }
        ...
        for (i = 0; i < ctLen - macLen; i += BLOCK_SIZE_AES)
        {
            ZERO128(tmp);
            ...
            {
                Byte2Word(tmp, ct, i, BLOCK_SIZE_AES);
                XOR128(GCTR_out, GCTR_out, tmp);
                Word2Byte(pt, i, GCTR_out, BLOCK_SIZE_AES);
            }
        }
